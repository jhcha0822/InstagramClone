...
4. 배포 시 발생한 이슈
   ...
   AWS에 배포하며 Google OAuth2가 도메인이 필요한 것을 확인했으나,  
   Amazone Route 53 서비스를 사용하기 위해 무료 도메인을 호스팅받으려 했지만 호스팅 업체 쪽 오류로 인해 방치한 상태입니다.  
   따라서 OAuth2는 로컬에서 테스트를 한 것으로 만족하고 마무리했습니다.  
